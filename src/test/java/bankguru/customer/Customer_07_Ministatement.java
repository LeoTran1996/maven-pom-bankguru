...
public class Customer_07_Ministatement extends BaseTest {
	...
	public void beforeClass(String browserName, String urlName) {
		...
		log.info("Pre-condition - Step 32: Click to 'Mini Statement' label");
		customerPage.openPageAtLeftSubMenuByText(driver, "Mini Statement");

		log.info("Pre-condition - Step 33: Verify 'Delete Account Form' heading");
		customerPage.isHeadinglDisplayedByTextName(driver, "Mini Statement Form");

	}

	@Test
	public void TC_01_Mini_Statement_AccountNo_Canot_Be_Empty() {

		log.info("Mini_Statement_AccountNo cannot be empty - Step 01: Input blank value to 'AccountNo' textbox");
		customerPage.sendKeyToTextboxByName(driver, "", "accountno");

		log.info("Mini_Statement_AccountNo cannot be empty - Step 02: Press key TAB to 'AccountNo' textbox");
		customerPage.sendKeyboardToElement(driver, BasePageUI.DYNAMIC_TEXTBOX_BY_NAME, Keys.TAB, "accountno");

		log.info("Mini_Statement_AccountNo cannot be empty - Step 03: Verify warning message is displayed with 'Account Number must not be blank'");
		verifyEquals(customerPage.getWarningMessageByID(driver, "2"), "Account Number must not be blank");

	}

	@Test
	public void TC_02_Mini_Statement_AccountNo_Must_Be_Numeric() {

		log.info("Mini_Statement_AccountNo cannot be numeric - Step 01:  Input numeric to 'AccountNo' textbox with value: 'anhtran123'");
		customerPage.sendKeyToTextboxByName(driver, "anhtran123", "accountno");

		log.info("Mini_Statement_AccountNo cannot be numeric - Step 02: Verify warning message is displayed with 'Characters are not allowed'");
		verifyEquals(customerPage.getWarningMessageByID(driver, "2"), "Characters are not allowed");
	}

	@Test
	public void TC_03_Mini_Statement_AccountNo_Can_Not_Have_Special_Characters() {

		log.info("Mini_Statement_AccountNo cannot have special charater - Step 01:  Input to 'AccountNo' textbox with value: '123@#'");
		customerPage.sendKeyToTextboxByName(driver, "123@#", "accountno");

		log.info("Mini_Statement_AccountNo cannot have special charater - Step 02: Verify warning message is displayed with 'Special characters are not allowed'");
		verifyEquals(customerPage.getWarningMessageByID(driver, "2"), "Special characters are not allowed");
	}

	@Test
	public void TC_04_Mini_Statement_AccountNo_Valid_AccountNo() {
		log.info("Mini_Statement_AccountNo Valid AccountNo  - Step 01: Input valid AccountNo to 'AccountNo' textbox");
		customerPage.sendKeyToTextboxByName(driver, accountNo, "accountno");

		log.info("Mini_Statement_AccountNo Valid AccountNo  - Step 02: Click to Submit button");
		customerPage.clickToButtonByName(driver, "AccSubmit");

		log.info("Mini_Statement_AccountNo Valid AccountNo  - Step 03: Verify 'Edit Account Entry Formr' heading is displayed");
		verifyTrue(customerPage.isMiniStateMessageDisplayed());
